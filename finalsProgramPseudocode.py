...
def simplify(adult, child, senior, family):
    ...
    # variables used to determine the value of children, adults, and seniors
    adultSeniorMath = math.floor((adult + senior) / 2)
    childMath = math.floor(child / 3)

    # better value in family tickets
    if ((adult + senior) >= 2) and (child >= 3):
        familyConvertQuery = YorN(
            f"Would you like to buy Family Ticket(s) instead of {adult} Adult, {senior} Senior, and {child} Children tickets? You will save at least $8 per ticket this way! ('y' / 'n') ")
        if familyConvertQuery == 'y':
            ...
            for b in range(familyConvertAmount):
                ...
                if adult >= 2:
                    ...
                    senior -= 2
                elif ((adult == 1) and (senior >= 1)) or ((adult >= 1) and (senior == 1)):
                    adult -= 1
                    senior -= 1
                else:
                    break
    ...
    # better value in group tickets
    if (senior != 4) or (child != 2) or (adult != 0):
        groupConvertQuery = ''
        if (adult + senior) >= 6:
            groupConvertQuery = YorN(
                f"Would you like to buy Group Ticket(s) instead of {adult} Adults and {senior} Senior tickets? You will save at least $6 per ticket this way! ('y' / 'n') ")
        ...
        # better value in group tickets in a specific circumstance
        elif ((adult + senior) == 4) and (child == 2) and (adult != 1):
            groupConvertQuery = YorN(
                f"Would you like to buy a Group Ticket instead of {adult} Adults, {senior} Senior, and {child} Child tickets? You will save at least $6 this way! ('y' / 'n') ")
            if groupConvertQuery == 'y':
                while True:
                    if adult > 0:
                        adult -= 1
        ...
        # better value in group tickets in another specific circumstance
        elif ((adult + senior) == 5) and (child == 1):
            groupConvertQuery = YorN(
                f"Would you like to buy a Group Ticket instead of {adult} Adults, {senior} Senior, and {child} Child tickets? You will save at least $2 this way! ('y' / 'n') ")
            if groupConvertQuery == 'y':
                while True:
                    if adult > 0:
                        adult -= 1
        ...
        if (groupConvertQuery == 'n') and (adultCount >= groupAdultCount) and (
    ...
    # changes the global variables to the local variables
    adultCount = adult
    childCount = child
    seniorCount = senior
    familyCount = family
...
def userInputChecker(repeatCheck):
    ...
    while True:
        # if they continue the current booking, booking-specific variables are unchanged/saved
...
# the 'repeat' value is a boolean that represents the current booking
def userInput():
    global repeat
...
# runs the main program forever until it is manually stopped or interrupted
while True:
    userInput()
